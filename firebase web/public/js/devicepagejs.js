...
let deviceSelect = document.querySelector('#deviceSelect')
    ...
    if(Cookies.get('user')){
        ...
        onValue(dbref, (snapshot) => {
            snapshot.forEach((devicetag) => {
                ...
                }else if(devicetag.key != "3stat" && devicetag.key != "strip" && devicetag.key != "bme" && devicetag.key != "moi" && devicetag.key != "drlk" && devicetag.key != "cam"){
...
function resetPage(){
    let deviceplace = document.querySelector('#deviceplace')
    let br = document.createElement('br')
    deviceplace.innerHTML = '';
    var now = swiper.realIndex;
    const user = Cookies.get('user')
            ...
            snapshot.forEach((devicetag) => {
                ...
                if (devicetag.key.substr(0, 2) == "裝置"){
                    ...
                    .then((deviceInfo)=>{
                        let devicestop = deviceInfo.val()
                        let mac = devicestop.replace('"', '').split(" ")[1]
                        let device = devicestop.replace('"', '').replace('"', '').split(" ")[2]
                        if(device == deviceSelect.value){
                            addItems(devicestop, a);
                        }
...
deviceSelect.addEventListener('change', resetPage)

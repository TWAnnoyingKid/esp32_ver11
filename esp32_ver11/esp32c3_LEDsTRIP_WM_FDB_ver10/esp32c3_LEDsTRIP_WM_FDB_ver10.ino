...
void setup() {
  ...
  if (Firebase.RTDB.getString(&fbdo_ALL, CONTROLLER)) {
    if (fbdo_ALL.stringData() != "0"){
      ...
      Serial.println("MAC：" + controller_MAC);
...
////////////////////////////////////////////////////////////////
      if (fbdo_ALL.stringData().substring(2, 21) == "CONTROLBYCONTROLLER"){
      // if (fbdo_ALL.dataPath() == "/CONTROLBYCONTROLLER"){
        if (fbdo_ALL.stringData().substring(24, 25) == "1") {
          if(controller_MAC != ""){
        ...
        else if (fbdo_ALL.stringData().substring(24, 25) == "0") {
...
void controlbyMAC(){
  if (Firebase.ready() && signupOK) {
    ...
    if (fbdo_control.streamAvailable()) {
      Serial.println("path:" + fbdo_control.dataPath());
      String test = fbdo_control.stringData();
      Serial.println("data:" + test);
